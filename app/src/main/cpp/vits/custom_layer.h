...
public:
    ...
    virtual int forward(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt) const {
        ...
        Mat rel_logits = _matmul_with_relative_keys(div(query, sqrt(k_channels), opt), key_relative_embeddings, opt);
        ...
        output = matplus(output, _matmul_with_relative_values(relative_weights, value_relative_embeddings, opt), opt);
